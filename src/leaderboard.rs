...
use poise::serenity_prelude::{CacheHttp, ChannelId, Context, CreateMessage, MessageBuilder, UserId};
use sqlx::{pool::PoolConnection, types::time::{OffsetDateTime, Time}, Acquire, Sqlite};
use time::Date;
...
pub fn leaderboard_new_month_job(ctx: &Context, data: &Data) -> anyhow::Result<Job> {
    let db = data.db_pool.clone();
    let http = ctx.http.clone();
    let channel_id = data.config.channels.leaderboard_announcement;

    Ok(Job::new_async("0 0 0 1 * *", move |_, _| {
        let db = db.clone();
        let http = http.clone();

        Box::pin(async move {
            leaderboard_new_month(&mut db.acquire().await.unwrap(), http, channel_id).await.unwrap()
        })
    })?)
}

async fn leaderboard_new_month(db: &mut PoolConnection<Sqlite>, http: impl CacheHttp, channel_id: ChannelId) -> anyhow::Result<()> {
    // TODO Ping Newsfeed role

    let month_start = real_leaderboard_start_datetime();
    let leaderboard = fetch_leaderboard(&mut db.acquire().await.unwrap(), month_start, Some(10)).await?;
    ...
    let guild =
        http.http().get_guild(
            ...
            .guild().ok_or(anyhow!("Cannot get channel as guild channel."))?
            .guild_id).await?;
        ...
        for ((uid, _), rewards) in top_with_rewards.iter() {
            for reward in rewards {
                ...
                reward.give_to_member(
                    &mut tx,
                    http.http(),
                    &guild.member(http.http(), *uid).await.expect("Cannot retrieve member.")).await?;
            }
        }
    }
    ...
    ", month_start)
        .fetch_all(db.acquire().await.unwrap())
        .await?
        .iter()
    ...
    let message = CreateMessage::new()
        .content({
            ...
            b.push_line(", and try our very best. Now is your chance. I believe in you.");
